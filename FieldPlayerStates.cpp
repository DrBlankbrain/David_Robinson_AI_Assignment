...
void ChaseBall::Execute(FieldPlayer* player)
{
	...
	if (player->Team()->checkAllPlayersAIHarrasing())
	{
		player->Team()->Opponents()->AllPlayersChase();
	}
	else {
...
{ 
  ...
  if (player->Team()->CanShoot(player->Ball()->Pos(),
	  power,
	  BallTarget) ||
	  (RandFloat() < Prm.ChancePlayerAttemptsPotShot)/* && (player->ID() < 5 && player->ID >= 0)*/)
  {
#ifdef PLAYER_STATE_INFO_ON
...
//	  power,
//	  BallTarget) ||
//	  (RandFloat() < Prm2.ChancePlayerAttemptsPotShot2)) && (player->ID() < 10 && player->ID > 4))
//  {
//#ifdef PLAYER_STATE_INFO_ON
//	  debug_con << "Player " << player->ID() << " attempts a shot at " << BallTarget << "";
//#endif
...
void Dribble::Execute(FieldPlayer* player)
{
	...
	if (dot < 0)
	{
		...
		//player will wait for team to catch up
		if (player->Team()->OpponentsGoal()->Center().x < player->Pos().x + 0.5f)
		{
			double x = rand() / static_cast<int>(RAND_MAX + 1);
			int randomValue = 0 + static_cast<int>(x * (1 - 0));
			if (randomValue == 1)
			{
				player->GetFSM()->ChangeState(KickBall::Instance());
			}
