...
void SoccerPitch::Update()
{
  ...
  {
	  if (m_pBlueGoal->NumGoalsScored() >= m_pRedGoal->NumGoalsScored() + 2)
	  {
		  m_pBlueTeam->ChangeRole(2, false);
	  }
	  else
	  {
		  m_pBlueTeam->ChangeRole(2, true);
	  }


	  if (m_pRedGoal->NumGoalsScored() >= m_pBlueGoal->NumGoalsScored() + 2)
	  {
		  m_pRedTeam->ChangeRole(7, false);
	  }
	  else
	  {
		  m_pRedTeam->ChangeRole(7, true);
	  }

	  if (m_pBlueGoal->NumGoalsScored() + 3 <= m_pRedGoal->NumGoalsScored())
	  {
		  m_pBlueTeam->ChangeRole(3, false);
	  }
	  else
	  {
		  m_pBlueTeam->ChangeRole(3, true);
	  }


	  if (m_pRedGoal->NumGoalsScored() + 3 <= m_pBlueGoal->NumGoalsScored())
	  {
		  m_pRedTeam->ChangeRole(8, false);
	  }
	  else
	  {
		  m_pRedTeam->ChangeRole(8, true);
	  }
